...
export default function InventoryPage() {
  ...
  const handleDeleteInventory = async () => {
    ...
    if (
      !window.confirm(
        "Remove the saved fleet inventory from this server? Uploaded SSH keys under /inventory are not deleted.",
      )
    ) {
      return;
    }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        {existing ? (
          <button
            ...
            className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border border-destructive/50 bg-card px-4 py-2.5 text-sm font-medium text-destructive shadow-sm hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <Trash2 className="h-4 w-4" />
            {deleting ? "Clearing…" : "Clear saved inventory"}
          </button>
        ) : null}
      </div>
      ...
      {existing ? (
        <p className="text-xs text-muted-foreground">
          <strong>Clear saved inventory</strong> removes the fleet document from the server (nodes, SSH settings, probe
          results). Uploaded SSH keys are kept; use key upload to manage those.
        </p>
      ) : null}
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        {status?.configured && (
          <button
            ...
            onClick={() => void handleDeleteInventory()}
            disabled={deleting || saving}
            className="inline-flex items-center gap-2 rounded-lg border border-destructive/40 px-4 py-3 text-sm font-medium text-destructive hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <Trash2 className="h-4 w-4" />
