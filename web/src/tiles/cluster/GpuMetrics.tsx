...
export default function GpuMetrics() {
  ...
  if (gpu.length === 0) {
    return (
      <div className="rounded-xl border border-slate-700 bg-slate-800/60 p-8 text-center space-y-2">
        <p className="text-slate-300 font-medium">No GPU metrics yet</p>
        <p className="text-slate-500 text-sm">
          The first collection sweeps GPU + NIC in parallel across all nodes (~1–2 min for 165 nodes).
          Turn on <span className="text-indigo-400">Live updates</span> or click <span className="text-indigo-400">Refresh</span> to trigger a sweep.
        </p>
      </div>
    );
  }
