    ...
    <div className="space-y-6">
      ...
      {status && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <KpiCard
            label="Healthy Nodes"
            value={String(status.healthy_nodes)}
            sub={
              status.unhealthy_nodes > 0
                ? `${status.unhealthy_nodes} unhealthy`
                : undefined
            }
            accent={status.healthy_nodes === status.total_nodes ? "green" : undefined}
          />
          <KpiCard
            label="Unreachable"
            value={String(status.unreachable_nodes)}
            accent={status.unreachable_nodes > 0 ? "red" : undefined}
          />
          <KpiCard label="Total GPUs" value={String(status.total_gpus)} />
          <KpiCard
            label="Avg GPU Util"
            value={status.total_gpus > 0 ? `${status.avg_gpu_util_pct.toFixed(1)}%` : "—"}
            sub={
              status.total_gpus > 0
      ...
      {gpu.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border p-8 text-center text-muted-foreground">
          ...
          <p className="mt-1 text-xs">
            They populate on the next poll (default 60 s) when live updates are on.
          </p>
        </div>
      ) : (
        <>
