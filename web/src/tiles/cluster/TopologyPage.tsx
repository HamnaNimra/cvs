  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && !hasData && (
        <div className="rounded-xl border border-slate-700 bg-slate-800/60 p-8 text-center space-y-3">
          <svg className="mx-auto w-10 h-10 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
            <circle cx={12} cy={12} r={9} />
            <path strokeLinecap="round" d="M12 8v4M12 16h.01" />
          </svg>
          <p className="text-slate-300 font-medium">No LLDP neighbors found</p>
          <p className="text-slate-500 text-sm max-w-md mx-auto">
            LLDP requires <code className="bg-slate-700 px-1 rounded">lldpd</code> to be installed on your nodes.
            Install it via the <span className="text-indigo-400">Configuration → Package Installs</span> page, or run:
          </p>
          <code className="inline-block bg-slate-900 border border-slate-600 rounded px-3 py-1 text-sm text-slate-300">
