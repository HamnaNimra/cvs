...
// CollectLLDP runs lldpcli on every host concurrently and returns the
// per-host LLDP neighbor list. A missing lldpd returns an empty neighbors
// list (not an error) so the topology page can show a clean empty-state.
func CollectLLDP(ctx context.Context, r Runner, hosts []string) map[string]NodeLLDPData {
	out := make(map[string]NodeLLDPData, len(hosts))
	var mu sync.Mutex
	...
	for _, h := range hosts {
		...
		go func(host string) {
			defer wg.Done()
			raw, err := r.Run(ctx, host, cmdLLDP)
			nd := NodeLLDPData{Host: host}
			if err != nil {
				nd.Error = err.Error()
			} else {
				nd.Neighbors = parseLLDP(raw)
			}
			mu.Lock()
			out[host] = nd
			mu.Unlock()
...
// CollectNIC runs the NIC commands on every host concurrently (bounded by the
// pool's MaxParallel). The per-host commands run sequentially within a goroutine.
func CollectNIC(ctx context.Context, r Runner, hosts []string) map[string]NodeNICMetrics {
	out := make(map[string]NodeNICMetrics, len(hosts))
	var mu sync.Mutex
	var wg sync.WaitGroup
	for _, h := range hosts {
		wg.Add(1)
...
func collectHostNIC(ctx context.Context, r Runner, host string) NodeNICMetrics {
	m := NodeNICMetrics{Host: host}

	// `rdma link` is the reachability gate: if the first command errors the host
	// is unreachable/no-SSH, so record it and skip the rest.
	raw, err := r.Run(ctx, host, cmdRDMALink)
	if err != nil {
		m.Error = err.Error()
		return m
	}
	m.RDMALinks = parseRDMALinks(raw)

	// Remaining commands are best-effort: a missing tool leaves its section empty.
	if out, err := r.Run(ctx, host, cmdRDMAStat); err == nil {
		m.RDMAStats = parseRDMAStats(out)
	}
	if out, err := r.Run(ctx, host, cmdIPAddr); err == nil {
		m.Interfaces = parseIPAddr(out)
	}
	if out, err := r.Run(ctx, host, cmdIPSLink); err == nil {
		m.EthStats = parseEthStats(out)
	}
	if out, err := r.Run(ctx, host, cmdRDMARes); err == nil {
		m.RDMAResources = parseRDMARes(out)
	}
	return m
}
