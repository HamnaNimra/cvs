...
// fakeRunner returns canned output (or an error) per host.
type fakeRunner struct {
	out map[string]string
	err map[string]error
}

func (f fakeRunner) Run(_ context.Context, host, _ string) (string, error) {
...
func TestCollectGPU_PerHostErrorIsolated(t *testing.T) {
	r := fakeRunner{
		out: map[string]string{"good": sampleMetricWrapped},
		err: map[string]error{"bad": context.DeadlineExceeded},
	}
	res := CollectGPU(context.Background(), r, []string{"good", "bad"})
	if len(res["good"].GPUs) != 1 || res["good"].Error != "" {
		t.Fatalf("good host wrong: %+v", res["good"])
	}
	if res["bad"].Error == "" || len(res["bad"].GPUs) != 0 {
