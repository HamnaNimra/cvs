...
// shared SSH pool, each behind a TTL cache with a stale-on-error fallback.
...
// NewSoftwareService wires the software collectors to the shared stores.
func NewSoftwareService(store inventory.Store, keys *inventory.KeyStore, logger *slog.Logger) *SoftwareService {
	if logger == nil {
		logger = slog.Default()
	}
	return &SoftwareService{store: store, keys: keys, logger: logger, ttl: softwareTTL}
}

// poolForCollect builds a fresh SSH pool over the reachable hosts. Callers own
// closing the pool.
func (s *SoftwareService) poolForCollect() (*ssh.Pool, []string, error) {
	...
	if pool == nil {
		return nil, nil, fmt.Errorf("SSH not available: software info requires key-based auth")
	}
...
func (s *SoftwareService) collectGPUSoftware(ctx context.Context) (*GPUSoftwareSnapshot, error) {
	...
	results := collectGPUSoftwareAll(ctx, pool, hosts)
	nodes := make([]NodeGPUSoftware, 0, len(results))
	for _, h := range hosts {
...
func collectGPUSoftwareAll(ctx context.Context, r Runner, hosts []string) map[string]NodeGPUSoftware {
	out := make(map[string]NodeGPUSoftware, len(hosts))
	var mu sync.Mutex
	...
	for _, h := range hosts {
		...
		go func(host string) {
			defer wg.Done()
			m := collectHostGPUSoftware(ctx, r, host)
			mu.Lock()
			out[host] = m
			mu.Unlock()
		}(h)
...
func collectHostGPUSoftware(ctx context.Context, r Runner, host string) NodeGPUSoftware {
	m := NodeGPUSoftware{Host: host}
	raw, err := r.Run(ctx, host, cmdAMDSMIVersion)
	if err != nil {
		m.Error = err.Error()
		return m
	}
	m.Version = parseAMDSMIVersion(raw)
	if fw, err := r.Run(ctx, host, cmdAMDSMIFirmware); err == nil {
		m.Firmware = parseGPUFirmware(fw)
	}
	return m
}
...
func (s *SoftwareService) collectDevlink(ctx context.Context) (*DevlinkSnapshot, error) {
	...
	results := collectDevlinkAll(ctx, pool, hosts)
...
func collectDevlinkAll(ctx context.Context, r Runner, hosts []string) map[string]NodeDevlink {
	out := make(map[string]NodeDevlink, len(hosts))
	var mu sync.Mutex
	var wg sync.WaitGroup
	for _, h := range hosts {
		wg.Add(1)
		go func(host string) {
			...
			raw, err := r.Run(ctx, host, cmdDevlinkInfo)
			if err != nil {
				m.Error = err.Error()
			} else {
				m.Devices = parseDevlink(raw)
			}
			mu.Lock()
			out[host] = m
			mu.Unlock()
