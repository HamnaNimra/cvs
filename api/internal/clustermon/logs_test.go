...
func TestSearch_RejectsInvalidGrep(t *testing.T) {
	svc := NewLogsService(nil, nil, nil)
...
func TestCollectHostLogs_ErrorGate(t *testing.T) {
	r := fakeRunner{
		out: map[string]string{"good": "some amd log line"},
		err: map[string]error{"bad": context.DeadlineExceeded},
	}
	res := collectLogsAll(context.Background(), r, []string{"good", "bad"})
	if res["good"].Error != "" || res["good"].AMDLogs == "" {
