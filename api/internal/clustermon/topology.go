...
// NewTopologyService creates a topology service backed by the shared inventory.
func NewTopologyService(store inventory.Store, keys *inventory.KeyStore, logger *slog.Logger) *TopologyService {
	if logger == nil {
		logger = slog.Default()
	}
	return &TopologyService{store: store, keys: keys, logger: logger}
}
...
func (s *TopologyService) collect(ctx context.Context) (*TopologySnapshot, error) {
	...
	results := CollectLLDP(ctx, pool, hosts)

	nodes := make([]NodeLLDPData, 0, len(hosts))
	for _, h := range hosts {
		if nd, ok := results[h]; ok {
