...
// nicRunner returns canned output keyed by (host, command-prefix).
type nicRunner struct {
	hosts map[string]map[string]string // host -> cmd -> output
	fail  map[string]error             // host -> error on every command
}

func (n nicRunner) Run(_ context.Context, host, cmd string) (string, error) {
	if e, ok := n.fail[host]; ok {
		return "", e
	}
	byCmd := n.hosts[host]
	switch {
	case cmd == cmdRDMALink:
		return byCmd["rdma_link"], nil
	case strings.Contains(cmd, "rdma statistic"):
		return byCmd["rdma_stat"], nil
	case strings.Contains(cmd, "ip addr"):
		return byCmd["ip_addr"], nil
	case cmd == cmdIPSLink:
		return byCmd["ip_s_link"], nil
	case cmd == cmdRDMARes:
		return byCmd["rdma_res"], nil
	}
	return "", nil
}
...
func TestCollectNIC_PerHostErrorIsolated(t *testing.T) {
	r := nicRunner{
		hosts: map[string]map[string]string{
			"good": {
				"rdma_link": sampleRDMALink,
				"rdma_stat": sampleRDMAStat,
				"ip_addr":   sampleIPAddr,
				"ip_s_link": sampleIPSLink,
				"rdma_res":  sampleRDMARes,
			},
		},
		fail: map[string]error{"bad": context.DeadlineExceeded},
	}
	res := CollectNIC(context.Background(), r, []string{"good", "bad"})
	good := res["good"]
	if good.Error != "" || len(good.RDMALinks) != 2 || len(good.EthStats) != 1 || len(good.RDMAResources) != 2 {
		t.Fatalf("good host wrong: %+v", good)
	}
	if res["bad"].Error == "" || len(res["bad"].RDMALinks) != 0 {
		t.Fatalf("bad host should carry an error: %+v", res["bad"])
	}
}
